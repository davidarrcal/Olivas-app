...
class MeteoDatosController {
  async crear(req, res, next) { try { const r = await meteoService.crear(Number(req.params.fincaId), req.body); res.status(201).json(r); } catch(e) { next(e); } }
  ...
  async actualizar(req, res, next) { try { const r = await meteoService.actualizar(Number(req.params.id), req.body); res.json(r); } catch(e) { next(e); } }
