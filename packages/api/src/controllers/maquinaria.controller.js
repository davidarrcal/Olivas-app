...
class MaquinariaController {
  ...
  async obtenerPorId(req, res, next) { try { const r = await maquinariaService.obtenerPorId(Number(req.params.id)); if(!r) return res.status(404).json({error:'No encontrada'}); res.json(r); } catch(e) { next(e); } }
  async actualizar(req, res, next) { try { const r = await maquinariaService.actualizar(Number(req.params.id), req.body); res.json(r); } catch(e) { next(e); } }
  async eliminar(req, res, next) { try { await maquinariaService.eliminar(Number(req.params.id)); res.status(204).send(); } catch(e) { next(e); } }
