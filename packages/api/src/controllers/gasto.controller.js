...
class GastoController {
  ...
  async obtenerPorId(req, res, next) { try { const r = await gastoService.obtenerPorId(Number(req.params.id), req.user.id); if(!r) return res.status(404).json({error:'No encontrado'}); res.json(r); } catch(e) { next(e); } }
  async actualizar(req, res, next) { try { const r = await gastoService.actualizar(Number(req.params.id), req.body, req.user.id); res.json(r); } catch(e) { next(e); } }
  async eliminar(req, res, next) { try { await gastoService.eliminar(Number(req.params.id), req.user.id); res.status(204).send(); } catch(e) { next(e); } }
